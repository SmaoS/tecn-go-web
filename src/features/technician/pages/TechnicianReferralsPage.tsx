...
export function TechnicianReferralsPage() {
  ...
  return <section><h2 className="mb-2 text-2xl font-bold">Invita y gana</h2>
    <p className="mb-5 text-slate-400">{technicianMode
      ? 'Invita clientes o técnicos a TecnGo. Si se registran con tu código y completan un servicio con calificación de 5 estrellas, ganas un servicio sin comisión.'
      : 'Invita amigos a descargar TecnGo y registrarse con tu código personal.'}</p>
      ...
      {code.data && <div className="grid gap-4">
        <article className="rounded-2xl border border-brand-500/40 bg-slate-900 p-6">
          ...
          <div className="flex flex-wrap gap-2"><button onClick={() => void copy()} className="rounded-lg bg-brand-500 px-4 py-2 font-bold text-slate-950">{copied ? 'Copiado' : 'Copiar código'}</button>
            <a href={`https://wa.me/?text=${encodeURIComponent(`Descarga TecnGo desde Google Play y regístrate con mi código ${code.data.code}: ${playStoreUrl}`)}`} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-700 px-4 py-2">Compartir</a></div>
        </article>
        ...
        {technicianMode && <article className="rounded-xl bg-slate-900 p-4"><h3 className="mb-3 font-bold">Beneficios</h3>{rewards.data?.length === 0 && <p className="text-slate-400">Aún no tienes beneficios.</p>}{rewards.data?.map((item) => <p key={item.id} className="border-t border-slate-800 py-2 text-sm">Servicio sin comisión · {item.status}</p>)}</article>}
      </div>}
    </QueryState>
  </section>
}

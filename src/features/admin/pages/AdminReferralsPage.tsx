...
export function AdminReferralsPage() {
  ...
  return <section><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><h2 className="text-2xl font-bold">Referidos</h2><span className={`rounded-full px-3 py-1 text-sm font-bold ${programEnabled ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300'}`}>{programEnabled ? 'Programa activo' : 'Programa inactivo'}</span></div>
    <QueryState pending={codes.isPending} error={codes.error}><div className="desktop-table-on-mobile"><table className="w-full text-left text-sm"><thead><tr className="text-slate-400"><th>Técnico</th><th>Código</th><th>Registrados</th><th>Calificados</th><th>Disponibles</th><th>Usados</th><th>Acciones</th></tr></thead><tbody>{codes.data?.map((item) => <tr key={item.id} className="border-t border-slate-800"><td data-label="Técnico" className="py-3">{item.technicianName}</td><td data-label="Código">{item.code}</td><td data-label="Registrados">{item.registered}</td><td data-label="Calificados">{item.qualified}</td><td data-label="Disponibles">{item.availableRewards}</td><td data-label="Usados">{item.usedRewards}</td><td data-label="Acciones" className="flex flex-wrap gap-2 py-2"><button onClick={() => setSelected(item)} className="text-brand-300">Ver historial</button><button onClick={() => action.mutate(() => adminApi.setReferralActive(item.id, !item.active))} className="text-amber-300">{item.active ? 'Desactivar' : 'Activar'}</button><button onClick={() => window.confirm('¿Regenerar este código? El código anterior dejará de funcionar.') && action.mutate(() => adminApi.regenerateReferral(item.id))} className="text-red-300">Regenerar</button></td></tr>)}</tbody></table></div></QueryState>

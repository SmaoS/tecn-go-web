    ...
    {open && <div className="mt-3 grid gap-4 lg:grid-cols-2">
      <div className="grid gap-2 rounded-xl bg-slate-950/50 p-3">
        ...
        <select className={inputClass} value={type} onChange={(event) => setType(event.target.value as EvidenceType)}>
          {['BEFORE_SERVICE', 'DURING_SERVICE', 'AFTER_SERVICE', 'DAMAGE_REPORT', 'OTHER'].map((value) => <option key={value}>{value}</option>)}
        </select>
        <input className={inputClass} type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" onChange={(event) => setFile(event.target.files?.[0])} />
        <input className={inputClass} placeholder="Descripción" value={description} onChange={(event) => setDescription(event.target.value)} />
        <button disabled={!file || action.isPending} onClick={() => file && action.mutate(() => serviceSupportApi.uploadEvidence(requestId, file, type, description))} className="rounded-lg bg-brand-500 p-2 font-bold text-slate-950">Subir evidencia</button>
        {evidence.data?.map((item) => <button key={item.id} onClick={() => void openPrivateFile(item.fileUrl)} className="text-left text-sm text-brand-300">{item.evidenceType} · {item.uploadedByName}</button>)}
      </div>
      <div className="grid gap-2 rounded-xl bg-slate-950/50 p-3">
        ...
        <button disabled={!file || !amount || action.isPending} onClick={() => file && action.mutate(() => serviceSupportApi.uploadProof(requestId, file, Number(amount), method))} className="rounded-lg bg-emerald-500 p-2 font-bold text-slate-950">Enviar comprobante</button>

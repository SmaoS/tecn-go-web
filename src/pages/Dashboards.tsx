...
import type { AdminDashboardSummary, ChatMessage, FinancialSummary, Payment, RequestStatus, ServiceCategory, ServiceQuote, ServiceRequest, SystemParameter, TechnicianLocation, TechnicianProfile, UnreadCount, UserNotification, UserProfile, UserVerification, VerificationStatus, Verifier } from '../types'
...
export function ClientDashboard() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const { data } = await api.post<ServiceRequest>('/v1/service-requests', {
        ...form,
      ...
      for (const image of serviceImages) {
        ...
        await api.post(`/v1/service-requests/${data.id}/images`, body)
      }
  ...
  async function rejectQuote(id: string, quoteId: string) {
    try { await api.put(`/v1/service-requests/${id}/quotes/${quoteId}/reject`); await load() } catch (reason) { setError(apiMessage(reason)) }
  }
  ...
  return <Shell title={`Hola, ${session?.fullName}`} subtitle="Panel cliente"><NotificationCenter /><button onClick={() => setShowProfile((value) => !value)} className="mb-6 rounded-lg border border-brand-500 px-4 py-2 text-brand-300">{showProfile ? 'Cerrar perfil' : 'Mi perfil'}</button>{showProfile && <UserProfileEditor />}<div className="grid gap-8 lg:grid-cols-2">
    ...
    <div><h2 className="mb-4 text-xl font-bold">Mis solicitudes</h2><div className="space-y-3">{requests.length === 0 && <p className="text-slate-400">Aún no tienes solicitudes.</p>}{requests.map((item) => <article key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
      ...
      {item.status === 'QUOTE_PENDING' && (quotes[item.id]?.length ?? 0) > 0 && <div className="mt-4 space-y-3"><strong className="text-sm">Cotizaciones recibidas</strong>{quotes[item.id].filter((quote) => quote.status === 'PENDING').map((quote) => <div key={quote.id} className="rounded-xl border border-slate-700 bg-slate-950/50 p-3"><Reputation photo={quote.technicianProfilePhotoUrl} name={quote.technicianName} rating={quote.technicianAverageRating} services={quote.technicianCompletedServicesCount} description={[quote.technicianExperienceDescription, quote.technicianCategories.join(', ')].filter(Boolean).join(' · ')} /><p className="mt-2 text-lg font-bold text-brand-400">${quote.price.toLocaleString()}</p><p className="text-xs text-slate-500">Expira: {new Date(quote.expiresAt).toLocaleString()}</p>{quote.description && <p className="text-sm text-slate-400">{quote.description}</p>}<div className="mt-3 flex gap-2"><button onClick={() => confirmQuote(item.id, quote.id)} className="rounded-lg bg-emerald-500 px-3 py-2 text-sm font-bold text-slate-950">Aceptar</button><button onClick={() => rejectQuote(item.id, quote.id)} className="rounded-lg border border-red-500 px-3 py-2 text-sm text-red-300">Rechazar</button></div></div>)}</div>}
...
const emptyProfile = { documentNumber: '', phone: '', categoryIds: [] as string[], description: '', profilePhotoUrl: '', documentPhotoUrl: '', certificatePhotoUrl: '', workExperienceDescription: '', latitude: '', longitude: '', homeAddress: '', homeLatitude: '', homeLongitude: '', homeCity: '', homeNeighborhood: '' }
...
export function TechnicianDashboard() {
  ...
  async function saveProfile(event: FormEvent) {
    event.preventDefault(); setError('')
    const payload = { ...profileForm, latitude: Number(profileForm.latitude), longitude: Number(profileForm.longitude), homeLatitude: Number(profileForm.homeLatitude), homeLongitude: Number(profileForm.homeLongitude) }
    try {
  ...
  useEffect(() => {
    ...
    const send = () => navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        if (!active) return
        void api.put('/v1/technicians/me/location', {
          latitude: coords.latitude, longitude: coords.longitude,
          accuracy: coords.accuracy, speed: coords.speed, heading: coords.heading, online: true,
        }).catch((reason) => setError(apiMessage(reason)))
      },
  ...
  return <Shell title={`Hola, ${session?.fullName}`} subtitle="Panel técnico">
    <NotificationCenter />
    <div className="mb-6 flex flex-wrap gap-2"><button onClick={() => setShowProfile((value) => !value)} className="rounded-lg border border-brand-500 px-4 py-2 text-brand-300">{showProfile ? 'Cerrar perfil' : 'Mi perfil'}</button>{profile?.status === 'APPROVED' && <button onClick={() => locationOnline ? void stopLocation() : setLocationOnline(true)} className={`rounded-lg px-4 py-2 font-bold ${locationOnline ? 'bg-emerald-500 text-slate-950' : 'border border-slate-700'}`}>{locationOnline ? 'Ubicación activa · Desactivar' : 'Activar ubicación'}</button>}{!session?.emailVerified && <button onClick={() => void sendEmailVerification()} className="rounded-lg border border-amber-500 px-4 py-2 text-amber-300">Reenviar verificación de correo</button>}</div>
    ...
    <div className={`grid gap-8 ${showProfile || !profile ? 'lg:grid-cols-2' : ''}`}>
      {(showProfile || !profile) && <form onSubmit={saveProfile} className="space-y-3 rounded-3xl border border-slate-800 bg-slate-900 p-6"><div className="flex justify-between"><h2 className="text-xl font-bold">Perfil técnico</h2>{profile && <StatusText value={profile.status} />}</div>
        ...
        <input placeholder="Dirección de domicilio" value={profileForm.homeAddress} onChange={(e) => setProfileForm({ ...profileForm, homeAddress: e.target.value })} required />
        <div className="grid grid-cols-2 gap-3"><input placeholder="Ciudad" value={profileForm.homeCity} onChange={(e) => setProfileForm({ ...profileForm, homeCity: e.target.value })} /><input placeholder="Barrio" value={profileForm.homeNeighborhood} onChange={(e) => setProfileForm({ ...profileForm, homeNeighborhood: e.target.value })} /></div>
        <div className="grid grid-cols-2 gap-3"><input type="number" step="any" placeholder="Latitud domicilio" value={profileForm.homeLatitude} onChange={(e) => setProfileForm({ ...profileForm, homeLatitude: e.target.value })} required /><input type="number" step="any" placeholder="Longitud domicilio" value={profileForm.homeLongitude} onChange={(e) => setProfileForm({ ...profileForm, homeLongitude: e.target.value })} required /></div>
    ...
    {profile?.status === 'APPROVED' && <div className="mt-8"><div className="mb-4 flex items-center gap-3"><label>Radio (km)</label><input className="max-w-28" type="number" min="1" max="100" value={radiusKm} onChange={(e) => setRadiusKm(e.target.value)} /><button onClick={() => void load()} className="rounded-lg border border-slate-700 px-3 py-2">Buscar</button></div>
      <section><h2 className="mb-4 text-xl font-bold">Solicitudes cercanas</h2><div className="space-y-3">{available.length === 0 && <p className="text-slate-400">No hay solicitudes dentro del radio.</p>}{available.map((item) => <article key={item.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">{item.firstServiceImageUrl && <img src={item.firstServiceImageUrl} alt="" className="mb-3 h-40 w-full rounded-xl object-cover" />}<strong>{item.categoryName}</strong><Reputation photo={item.clientProfilePhotoUrl} name={item.clientName} rating={item.clientAverageRating} services={item.clientPaidServicesCount} /><p className="mt-2 text-sm text-slate-400">{item.description}</p>{item.images?.length > 0 && <ImageGallery urls={item.images.map((image) => image.imageUrl)} />}{item.estimatedPrice != null && <p className="mt-2 font-bold text-brand-400">Estimado del cliente: ${item.estimatedPrice.toLocaleString()}</p>}<p className="mt-2 text-xs text-slate-500">{item.address} · {item.distanceKm?.toFixed(2)} km</p><div className="mt-4 grid gap-2 sm:grid-cols-[1fr_2fr_auto]"><input type="number" min="1" placeholder="Tu cotización" value={quotes[item.id] ?? ''} onChange={(e) => setQuotes({ ...quotes, [item.id]: e.target.value })} /><input placeholder="Descripción de la oferta (opcional)" value={quoteDescriptions[item.id] ?? ''} onChange={(e) => setQuoteDescriptions({ ...quoteDescriptions, [item.id]: e.target.value })} /><button disabled={!quotes[item.id]} onClick={() => quote(item.id)} className="rounded-lg bg-brand-500 px-3 py-2 font-bold text-slate-950 disabled:opacity-50">Cotizar</button></div></article>)}</div></section>
    </div>}
...
function UserProfileEditor() {
  ...
  function useHomeLocation() {
    navigator.geolocation.getCurrentPosition(({ coords }) => {
      if (profile) setProfile({ ...profile, homeLatitude: coords.latitude, homeLongitude: coords.longitude })
    }, () => setError('No fue posible obtener la ubicación del navegador'))
  }
  if (!profile) return null
  return <form onSubmit={save} className="mb-8 rounded-2xl border border-slate-800 bg-slate-900 p-5"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-bold">Mi perfil y reputación</h2><span className="text-brand-400">★ {profile.averageRating.toFixed(1)} · {profile.paidServicesCount} pagados</span></div><div className="mt-2"><VerificationBadge value={profile.verificationStatus} /><p className="mt-1 text-sm text-slate-400">Correo: {profile.emailVerified ? 'verificado' : 'pendiente'} · Documentos: {profile.documentsVerified ? 'verificados' : 'pendientes'}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={profile.fullName} onChange={(event) => setProfile({ ...profile, fullName: event.target.value })} required /><input placeholder="Teléfono" value={profile.phone ?? ''} onChange={(event) => setProfile({ ...profile, phone: event.target.value })} /><input placeholder="Dirección de domicilio" value={profile.homeAddress ?? ''} onChange={(event) => setProfile({ ...profile, homeAddress: event.target.value })} /><input placeholder="Ciudad" value={profile.homeCity ?? ''} onChange={(event) => setProfile({ ...profile, homeCity: event.target.value })} /><input placeholder="Barrio" value={profile.homeNeighborhood ?? ''} onChange={(event) => setProfile({ ...profile, homeNeighborhood: event.target.value })} /><div className="grid grid-cols-2 gap-2"><input type="number" step="any" placeholder="Latitud" value={profile.homeLatitude ?? ''} onChange={(event) => setProfile({ ...profile, homeLatitude: Number(event.target.value) })} /><input type="number" step="any" placeholder="Longitud" value={profile.homeLongitude ?? ''} onChange={(event) => setProfile({ ...profile, homeLongitude: Number(event.target.value) })} /></div><label className="text-sm">Foto de perfil<input type="file" accept=".jpg,.jpeg,.png" onChange={(event) => void file('profilePhotoUrl', event.target.files?.[0])} /></label><label className="text-sm">Documento de identidad<input type="file" accept=".jpg,.jpeg,.png,.pdf" onChange={(event) => void file('documentPhotoUrl', event.target.files?.[0])} /></label></div>{error && <p className="mt-2 text-sm text-slate-300">{error}</p>}<div className="mt-3 flex flex-wrap gap-2"><button className="rounded-lg border border-brand-500 px-3 py-2 text-sm text-brand-300">Guardar perfil</button><button type="button" onClick={useHomeLocation} className="rounded-lg border border-slate-700 px-3 py-2 text-sm">Usar mi ubicación</button>{!profile.emailVerified && <button type="button" onClick={() => void sendEmailVerification()} className="rounded-lg border border-slate-700 px-3 py-2 text-sm">Verificar correo</button>}</div></form>
}

function ImageGallery({ urls }: { urls: string[] }) {
  return <div className="mt-3 grid grid-cols-3 gap-2">{urls.map((url) => <a key={url} href={url} target="_blank" rel="noreferrer"><img src={url} alt="Evidencia del servicio" className="h-24 w-full rounded-lg object-cover" /></a>)}</div>
}

function TechnicianLocationViewer({ requestId }: { requestId: string }) {
  const [location, setLocation] = useState<TechnicianLocation | null>(null)
  const load = useCallback(() => api.get<TechnicianLocation>(`/v1/service-requests/${requestId}/technician-location`)
    .then(({ data }) => setLocation(data)).catch(() => setLocation(null)), [requestId])
  usePolling(load, 10_000)
  if (!location) return <p className="mt-3 text-xs text-slate-500">Esperando ubicación del técnico...</p>
  return <div className="mt-3 rounded-xl bg-slate-950/50 p-3 text-sm"><strong>{location.online ? 'Técnico en línea' : 'Última ubicación conocida'}</strong><p className="text-slate-400">{location.latitude.toFixed(6)}, {location.longitude.toFixed(6)}</p><time className="text-xs text-slate-500">{new Date(location.updatedAt).toLocaleString()}</time></div>
}
...
function VerifierManager() {
  ...
  const [form, setForm] = useState({ fullName: '', email: '', password: '', homeAddress: '', homeCity: '', homeNeighborhood: '', homeLatitude: '', homeLongitude: '' })
  const [error, setError] = useState('')
  ...
  async function create(event: FormEvent) {
    event.preventDefault(); setError('')
    try {
      await api.post('/v1/admin/verifiers', {
        ...form,
      ...
      setForm({ fullName: '', email: '', password: '', homeAddress: '', homeCity: '', homeNeighborhood: '', homeLatitude: '', homeLongitude: '' })
      await load()
    } catch (reason) { setError(apiMessage(reason)) }
  }
...
export function AdminDashboard() {
  ...
  const load = useCallback(async () => { try { const [profiles, catalog, payments, dashboard, config, technicianLocations] = await Promise.all([api.get<TechnicianProfile[]>('/v1/admin/technicians/pending'), api.get<ServiceCategory[]>('/v1/admin/service-categories'), api.get<FinancialSummary>('/v1/admin/payments'), api.get<AdminDashboardSummary>('/v1/admin/dashboard'), api.get<SystemParameter[]>('/v1/admin/system-parameters'), api.get<TechnicianLocation[]>('/v1/admin/technicians/locations')]); setPending(profiles.data); setCategories(catalog.data); setFinances(payments.data); setSummary(dashboard.data); setParameters(config.data); setLocations(technicianLocations.data) } catch (reason) { setError(apiMessage(reason)) } }, [])
  useEffect(() => { void load() }, [load])
  usePolling(load, 10_000)
...
function SystemParametersPanel({ items, onSave }: {
  items: SystemParameter[]
  onSave: (item: SystemParameter, value: string) => Promise<void>
}) {
...
function TechnicianLocationsPanel({ items }: { items: TechnicianLocation[] }) {
  return <section className="mb-8"><h2 className="mb-3 text-xl font-bold">Ubicación de técnicos</h2><div className="grid gap-3 md:grid-cols-2">{items.length === 0 && <p className="text-slate-400">Aún no hay ubicaciones reportadas.</p>}{items.map((item) => <article key={item.technicianId} className="rounded-xl border border-slate-800 bg-slate-900 p-4"><div className="flex justify-between"><strong>{item.technicianName}</strong><span className={item.online ? 'text-emerald-400' : 'text-slate-500'}>{item.online ? 'Online' : 'Offline'}</span></div><p className="mt-2 text-sm text-slate-400">{item.latitude.toFixed(6)}, {item.longitude.toFixed(6)}</p><time className="text-xs text-slate-500">{new Date(item.updatedAt).toLocaleString()}</time></article>)}</div></section>
}
